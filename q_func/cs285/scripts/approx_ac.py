...
class Approx(object):
    ...
    def prepare_data(self, file_path):
        ...
        acs = [entry['actions'][0][0] for entry in data]
        next_obs = [entry['next_obs'] for entry in data]
        q_values = [entry['q_value'][0][0] for entry in data]
        ...
        if task == "value":
            ...
            # X_train, X_test, y_train, y_test = train_test_split(obs, q_values, test_size=0.2, random_state=42)
            X_train = obs
            y_train = q_values
            X_test = obs
            y_test = q_values
    ...
    def fit_model(self, model, criterion, optimizer, train_loader, test_loader, total_variance):
        ...
        run = wandb.init(
                        ...
                        project="rl-approx-error-ac-" + str(self.params["n_layers"]) + "-" + str(self.params["size"]),
