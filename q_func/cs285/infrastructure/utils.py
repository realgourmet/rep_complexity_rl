...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        ...
        """Baihe Huang 08/31/2022"""
        get_path = sample_trajectory(env, policy, max_path_length, render)
        paths.append(get_path)
        timesteps_this_batch += get_pathlength(get_path)
        """Baihe Huang 08/31/2022"""
...
def eval_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        action = policy.get_action(ob, sample=False)
        """
        Baihe: changed to sample = True, changed back
        """
