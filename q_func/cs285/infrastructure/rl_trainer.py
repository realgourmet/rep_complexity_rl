...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            ###################
            if self.params['save_params'] and itr % (10 * print_period) == 0:
                print("saving checkpoints...")
                self.agent.save_actor('{}/agent_itr_{}.pt'.format(self.params['logdir'], itr))
                self.agent.save_critic('{}/critic_itr_{}.pt'.format(self.params['logdir'], itr))


    def run_eval_loop(self, n_iter, collect_policy, eval_policy,
                          initial_expertdata=None, relabel_with_expert=False,
                          start_relabel_with_expert=1, expert_policy=None):
        """
        :param n_iter:  number of (dagger) iterations
        ...
        rollout_data_path = 'rollout/' + self.params['rollout_path'] + '.pkl'
        save_path = rollout_data_path
        ...
        for itr in range(n_iter):
            ...
            # train agent (using sampled data from replay buffer)
            if itr % print_period == 0 and itr > 0:
                print("\nRolling agent...")
                # all_logs = self.train_agent()
                ...
                obs = ptu.from_numpy(np.array(ob_batch))
                action = np.array(ac_batch)
                # action = ptu.from_numpy(np.array(ac_batch)).to(torch.long)

                length = obs.size(dim=0)

                # action = ptu.to_numpy(action)
                next_obs = np.array(next_ob_batch)
                rews = np.array(re_batch)

                print(obs.size(), action.shape, next_obs.shape, rews.shape, length)

                # for DQN
                # qa_t_values = self.agent.critic.q_net(obs)
                # q_t_values = torch.gather(qa_t_values, 1, action.unsqueeze(1)).squeeze(1)

                # obs = ptu.to_numpy(obs)
                # action = ptu.to_numpy(action)
                # next_obs = ptu.to_numpy(next_obs)
                q_value = self.agent.critic.critic_network(obs)
                q_value = ptu.to_numpy(q_value.detach())

                obs = ptu.to_numpy(obs)

                # q_value = ptu.to_numpy(q_t_values.detach())
                # rews = ptu.to_numpy(rews)
        ...
        # Serialize and save the updated data using pickle in append mode
    ...
    def run_sac_training_loop(self, n_iter, collect_policy, eval_policy):
        ...
        for itr in range(n_iter):
            ...
            if itr==0:
                ...
                action = self.agent.actor.get_action(obs)
            ...
            if self.logvideo or self.logmetrics:
                ...
                ### SAVING THE CHECKPOINTS
                ###################
                if self.params['save_params'] and itr % print_period == 0:
                    self.agent.save_actor('{}/agent_itr_{}.pt'.format(self.params['logdir'], itr))
                    self.agent.save_critic('{}/critic_itr_{}.pt'.format(self.params['logdir'], itr))
    ...
    def run_sac_eval_loop(self, n_iter, collect_policy, eval_policy):
        """
        :param n_iter:  number of (dagger) iterations
        :param collect_policy:
        :param eval_policy:
        ...
        for itr in range(n_iter):
            ...
            if itr==0:
                ...
                action = ptu.from_numpy(np.array([action]))

                # print(obs, action)

                q_value = self.agent.critic.Q1(torch.cat((obs, action), 1))

                iteration_data = {
                    'observations': ptu.to_numpy(obs),
                    'actions': ptu.to_numpy(action),
                    'next_obs': next_obs,
                    'q_value': ptu.to_numpy(q_value.detach()),
                    'reward': rew
                }
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            # train_log = TODO
            """Baihe Huang 08/31/2022"""
            # print('action size from replay buffer', ptu.from_numpy(ac_batch).size())
